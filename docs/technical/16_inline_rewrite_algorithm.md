...
2. 仅当没有参数门控目标时，才从 `EXIT` 反向可达子图中提取入口根节点。
...
若目标集合为空（典型于仅 DATA/PORT 直达 `EXIT` 的子图），先注入隐式目标 `valueExit`，
再按上述统一规则收敛出唯一 `ctrlEntry`。

### 7.2 以 `EXIT` 为准的控制完成契约（无 `exitSync` 汇聚）

当前实现将控制完成锚点定义为：

- 默认使用 `valueExit`；
- 若 `EXIT` 存在 ctrl 输入，则使用该唯一 `EXIT.ctrlInputs[0]`。

验证器约束为：`EXIT.ctrlInputs.size() <= 1`。

因此 `inlineCallable` 不再通过插入 `exitSync` 做“多叶收敛”，而是直接从 `EXIT` 的结构
推导完成锚点；若该锚点与值出口不同，则在内部插入 `GATE` 产出受控值。
...
### 9.4 Builder/Mutation 层（`GraphBuilder` + `NodeMutation`）
...
4. **InlineResult 合法性**  
   `valueExit` 必须始终可用（必要时为内部门控后的 `GATE`）；并且目标图必须满足 `EXIT.ctrlInputs.size() <= 1` 契约。
