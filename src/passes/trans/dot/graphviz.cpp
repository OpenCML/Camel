...
std::string GraphVizDumpPass::dumpGraph(const GIR::graph_ptr_t &graph) {
    ...
    for (size_t i = 0; i < withPorts.size(); ++i) {
        auto portNode = tt::as_ptr<PortNode>(withPorts[i]);
        res += std::format(
    ...
    for (size_t i = 0; i < normPorts.size(); ++i) {
        auto portNode = tt::as_ptr<PortNode>(normPorts[i]);
        res += std::format(
            "{}{}{} -> {} [label=\"{}\", style=solid];\r\n",
            baseIndent_,
            indent_,
