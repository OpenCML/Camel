...
static std::pair<Array *, Array *> __py_array_to_camel_complex__(py::array py_arr, type_ptr_t tupleTypePtr, Context &ctx) {
    try {
        py::module_ np = py::module_::import("numpy");
        
        if (!tupleTypePtr || tupleTypePtr->code() != TypeCode::Tuple) {
            ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit("phot: expected tuple return type for complex arrays");
            return {nullptr, nullptr};
        }
        
        auto tupleType = tt::as_shared<TupleType>(tupleTypePtr);
        if (tupleType->types().size() != 2) {
            ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit("phot: expected tuple with 2 arrays (real, imag)");
            return {nullptr, nullptr};
        }
        ...
        if (is_complex) {
            ...
            arr_obj = np.attr("ascontiguousarray")(arr_obj, py::arg("dtype") = np.attr("complex128"));
            py::array py_real = np.attr("ascontiguousarray")(np.attr("real")(arr_obj), py::arg("dtype") = np.attr("float64"));
            py::array py_imag = np.attr("ascontiguousarray")(np.attr("imag")(arr_obj), py::arg("dtype") = np.attr("float64"));
        ...
        ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit(std::string("phot: Python error: ") + e.what());
        return {nullptr, nullptr};
    } catch (const std::exception &e) {
        ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit(std::string("phot: error: ") + e.what());
        return {nullptr, nullptr};
    }
}
...
void __phot_modulation__(GraphIR::data_idx_t self, data_arr_t nargs, data_arr_t, Frame &frame, Context &ctx) {
    ...
    try {
        ...
        auto [realArr_x, imagArr_x] = __py_array_to_camel_complex__(signal_obj_x.cast<py::array>(), xPolTupleType, ctx);
        ...
        auto [realArr_y, imagArr_y] = __py_array_to_camel_complex__(signal_obj_y.cast<py::array>(), yPolTupleType, ctx);
        
        if (!realArr_x || !imagArr_x || !realArr_y || !imagArr_y) {
            ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit("phot.modulation: failed to convert arrays");
            frame.set(self, NullSlot);
            return;
        }
        ...
        ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit(std::string("phot.modulation error: ") + e.what());
        frame.set(self, NullSlot);
    }
}
...
void __phot_up_sample__(GraphIR::data_idx_t self, data_arr_t nargs, data_arr_t, Frame &frame, Context &ctx) {
    ...
    Int up_sampling_factor = frame.get<Int>(nargs[1]);
    ...
    try {
        ...
        py::array py_x_complex = np.attr("array")(py_x_real + j * py_x_imag, py::arg("dtype") = np.attr("complex128"));
        ...
        py::array py_y_complex = np.attr("array")(py_y_real + j * py_y_imag, py::arg("dtype") = np.attr("complex128"));
        ...
        py::list py_result = phot.attr("up_sample")(signals_list, up_sampling_factor).cast<py::list>();
        ...
        auto [realArr_x, imagArr_x] = __py_array_to_camel_complex__(py_result_x, xPolTupleType, ctx);
        auto [realArr_y, imagArr_y] = __py_array_to_camel_complex__(py_result_y, yPolTupleType, ctx);
        
        if (!realArr_x || !imagArr_x || !realArr_y || !imagArr_y) {
            ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit("phot.up_sample: failed to convert arrays");
            frame.set(self, NullSlot);
            return;
        }
        ...
        ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit(std::string("phot.up_sample error: ") + e.what());
        frame.set(self, NullSlot);
    }
}
...
void __phot_pulse_shaper__(GraphIR::data_idx_t self, data_arr_t nargs, data_arr_t, Frame &frame, Context &ctx) {
    Tuple *signals_tuple = frame.get<Tuple *>(nargs[0]);
    Int up_sampling_factor = frame.get<Int>(nargs[1]);
    Double rolloff = frame.get<Double>(nargs[2]);
    Double baud = frame.get<Double>(nargs[3]);
    ...
    try {
        ...
        py::list py_result = phot.attr("pulse_shaper")(signals_list, up_sampling_factor, rolloff, baud).cast<py::list>();
        ...
        if (!realArr_x || !imagArr_x || !realArr_y || !imagArr_y) {
            ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit("phot.pulse_shaper: failed to convert arrays");
            frame.set(self, NullSlot);
            return;
        }
        ...
        ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit(std::string("phot.pulse_shaper error: ") + e.what());
        frame.set(self, NullSlot);
    }
}
...
void __phot_constellation_diagram__(GraphIR::data_idx_t self, data_arr_t nargs, data_arr_t, Frame &frame, Context &ctx) {
    Tuple *signals_tuple = frame.get<Tuple *>(nargs[0]);
    bool is_plot = (nargs.size > 1) ? frame.get<bool>(nargs[1]) : true;
    bool isdata = (nargs.size > 2) ? frame.get<bool>(nargs[2]) : false;
    ...
    try {
        ...
        phot.attr("constellation_diagram")(signals_list, py::arg("is_plot") = is_plot, py::arg("isdata") = isdata);
        ...
        ctx.rtmDiags()->of(RuntimeDiag::RuntimeError).commit(std::string("phot.constellation_diagram error: ") + e.what());
        frame.set(self, NullSlot);
    }
}
