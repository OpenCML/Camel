...
using camel::runtime::canonicalizeStaticSlotRecursive;
...
class GraphEncodeSession {
  public:
    GCGraph *encode(const graph_ptr_t &sourceGraph) {
        ...
        if (auto it = cache_.find(sourceGraph.get()); it != cache_.end()) {
            return it->second;
        }
        ...
        auto *runtimeGraph = runtime::GCGraphBuildAccess::create(
            ...
            {},
            {},
            runtime::GCGraphNativePayload{},
            {});
        cache_[sourceGraph.get()] = runtimeGraph;

        auto dependencies    = collectDependencies(sourceGraph);
        auto subGraphs       = collectSubGraphs(sourceGraph);
        auto payload         = buildPayload(sourceGraph);
        auto runtimeSlots    = buildStaticSlots(sourceGraph);
        auto staticGraphRefs = collectStaticGraphRefs(sourceGraph, runtimeSlots);

        runtime::GCGraphBuildAccess::rewrite(
            runtimeGraph,
            const_cast<TupleType *>(sourceGraph->staticDataType()),
            nullptr,
            dependencies,
  ...
  private:
    std::vector<GCGraph *> collectDependencies(const graph_ptr_t &sourceGraph) {
        std::vector<GCGraph *> dependencies;
        for (const auto &dep : sourceGraph->dependencies()) {
            if (dep) {
                dependencies.push_back(encode(dep));
            }
        }
        for (Node *node : sourceGraph->nodes()) {
            ...
            if (targetGraph &&
                std::find(dependencies.begin(), dependencies.end(), cache_[targetGraph.get()]) ==
                    dependencies.end()) {
                dependencies.push_back(encode(targetGraph));
            }
        }
        return dependencies;
    }

    std::vector<GCGraph *> collectSubGraphs(const graph_ptr_t &sourceGraph) {
        ...
        for (const auto &[_, nestedSubGraphs] : sourceGraph->subGraphs()) {
            for (const auto &subGraph : nestedSubGraphs) {
                if (subGraph) {
                    subGraphs.push_back(encode(subGraph));
                }
            }
        }
        return subGraphs;
    ...
    runtime::GCGraphNativePayload buildPayload(const graph_ptr_t &sourceGraph) {
        return runtime::buildCompileGraphNativePayload(
            sourceGraph,
    ...
    collectStaticGraphRefs(const graph_ptr_t &sourceGraph, std::span<const slot_t> runtimeSlots) {
        ...
        for (Node *node : sourceGraph->nodes()) {
            ...
            if (GCGraph *runtimeRef = resolveCompileStaticFunctionGraph(
                    funcObj,
                    sourceGraph,
                    [&](const graph_ptr_t &target) { return encode(target); },
    ...
    std::unordered_map<const Graph *, GCGraph *> cache_;
};
