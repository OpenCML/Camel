...
class PyplotExecutor : public Executor {
  public:
    ...
    void eval(std::string uri, GraphIR::node_ptr_t &self, Frame &frame) override {
        EXEC_WHEN_DEBUG(l.in("PyplotExec").debug("Evaluating operator of URI: {}", uri));
...
executor_ptr_t createPyplotExecutor(context_ptr_t ctx) {
    return std::make_shared<PyplotExecutor>(ctx, getPyplotOpsMap());
}
