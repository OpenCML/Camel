...
namespace {
class AlgoExecutor : public Executor {
  public:
    AlgoExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createAlgoExecutor(context_ptr_t ctx) {
    return std::make_shared<AlgoExecutor>(ctx, getAlgoOpsMap());
}
