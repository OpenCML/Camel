...
namespace {
class ReExecutor : public Executor {
  public:
    ReExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createReExecutor(context_ptr_t ctx) {
    return std::make_shared<ReExecutor>(ctx, getReOpsMap());
}
