...
class MathExecutor : public Executor {
  public:
    MathExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createMathExecutor(context_ptr_t ctx) {
    return std::make_shared<MathExecutor>(ctx, getMathOpsMap());
}
