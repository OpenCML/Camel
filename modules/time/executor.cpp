...
namespace {
class TimeExecutor : public Executor {
  public:
    TimeExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createTimeExecutor(context_ptr_t ctx) {
    return std::make_shared<TimeExecutor>(ctx, getTimeOpsMap());
}
