...
namespace {
class OSExecutor : public Executor {
  public:
    OSExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createOSExecutor(context_ptr_t ctx) {
    return std::make_shared<OSExecutor>(ctx, getOSOpsMap());
}
