...
namespace {
class StatisticsExecutor : public Executor {
  public:
    StatisticsExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createStatisticsExecutor(context_ptr_t ctx) {
    return std::make_shared<StatisticsExecutor>(ctx, getStatisticsOpsMap());
}
