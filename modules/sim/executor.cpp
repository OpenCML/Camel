...
class SimExecutor : public Executor {
  public:
    SimExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createSimExecutor(context_ptr_t ctx) {
    return std::make_shared<SimExecutor>(ctx, getSimOpsMap());
}
