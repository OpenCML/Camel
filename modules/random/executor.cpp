...
class RandomExecutor : public Executor {
  public:
    RandomExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createRandomExecutor(context_ptr_t ctx) {
    return std::make_shared<RandomExecutor>(ctx, getRandomOpsMap());
}
