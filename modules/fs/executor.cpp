...
class FsExecutor : public Executor {
  public:
    FsExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createFsExecutor(context_ptr_t ctx) {
    return std::make_shared<FsExecutor>(ctx, getFsOpsMap());
}
