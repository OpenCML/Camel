...
namespace {
class CryptoExecutor : public Executor {
  public:
    CryptoExecutor(context_ptr_t ctx, std::unordered_map<std::string, operator_t> ops)
...
executor_ptr_t createCryptoExecutor(context_ptr_t ctx) {
    return std::make_shared<CryptoExecutor>(ctx, getCryptoOpsMap());
}
