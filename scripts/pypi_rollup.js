...
function main() {
    ...
    const CAMEL_PKG_BASE = path.join(PYPI_BASE, 'src', 'camel')

    copyDir(path.join(BASEDIR, 'build', 'Release'), path.join(CAMEL_PKG_BASE, 'bin'))
    copyDir(path.join(BASEDIR, 'stdlib'), path.join(CAMEL_PKG_BASE, 'bin', 'stdlib'))
    ...
    removeDir(path.join(CAMEL_PKG_BASE, 'bin'))
    logDone('Cleaned up')
    process.chdir(BASEDIR)
}
